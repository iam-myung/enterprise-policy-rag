...
# 新源码根（相对工程根 enterprise_policy_rag/）
...
def project_root() -> Path:
    ...
    legacy_guard.py 位于 <root>/enterprise_policy_rag/tools/，向上三级即项目根。
    """
    return Path(__file__).resolve().parent.parent.parent


def engine_root() -> Path:
    """新工程根目录 <root>/enterprise_policy_rag。"""
...
def cmd_gen(root: Path, mpath: Path) -> int:
    ...
    manifest = {
        ...
        "description": "旧 CASE 只读基线（SPEC §1.1）。记录每个旧文件相对项目根的路径/大小/SHA-256。",
...
def cmd_check_refs(eng_root: Path) -> int:
    ...
    for pf in py_files:
        ...
        if "sys.path" in text:
            problems.append(f"{rel}: 出现 sys.path 操作（疑似运行时注入旧 CASE）")
